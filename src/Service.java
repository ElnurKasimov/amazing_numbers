...
public class Service {
    ...
    public static void processAsMany (String[] parameters) {
        ...
        if (firstNumber <= 0) {
            ...
            List<String> propertiesToAvoid = new ArrayList<>();
            for (String property : properties) {
                if (property.charAt(0) == '-') {
                    propertiesToAvoid.add(property.substring(1));
                } else {
            ...
            while (counter < secondNumber) {
                Map<String, String> numberProperties = getProperties(firstNumber);
                if (propertiesToAvoid.isEmpty()) {
                    if (containsSuchProperties(numberProperties, propertiesToAdd)) {
                        counter++;
                    ...
                    if (containsSuchProperties(numberProperties, propertiesToAdd)) {
                        if (! containsSuchProperties(numberProperties, propertiesToAvoid)) {
                            counter++;
                            System.out.print(firstNumber + " is ");
                            System.out.println(getPropertiesString(getProperties(firstNumber)));
                        }
    ...
    private static Map<String, String> checkPropertyMutuallyExclusive(List<String> properties) {
        ...
        for (String property : properties) {
            if (properties.contains(property) && properties.contains(property.substring(1))) {
                result.put(property, property.substring(1));
            }
        }
        return  result;
    }
